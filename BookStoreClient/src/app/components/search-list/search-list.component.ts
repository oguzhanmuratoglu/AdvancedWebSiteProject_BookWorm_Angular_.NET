...
export class SearchListComponent {
  ...
  )
  {
      this.route.queryParams.subscribe(params => {
        ...
        this.selectedCategory = params['anotherParam']; 
      });


      console.log(this.searchItem, this.selectedCategory)
  ...
  getBooksByFilter(){
      ...
      this.filterRequestModel.searchTerm = this.searchItem;


    this.http.post<FilterRequestModel>("https://localhost:7062/api/Books/GetBooksBySearch", this.filterRequestModel)
    .subscribe({
      next: (res:any) => {
        debugger;
        this.response = res;
